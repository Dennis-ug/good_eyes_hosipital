...
import { Patient, CreatePatientRequest } from '@/lib/types'
...
export default function EditPatientPage() {
    ...
    const fetchPatient = async () => {
      try {
        ...
        setPatient({
          ...
          ageInYears: patientData.ageInYears || 0,
          ageInMonths: patientData.ageInMonths || 0,
          ...
          foreignerOrRefugee: patientData.foreignerOrRefugee || '',
          nonUgandanNationalIdNo: patientData.nonUgandanNationalIdNo || '',
          residence: patientData.residence || '',
          researchNumber: patientData.researchNumber || ''
        })
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedPatient = await patientApi.update(patientId, patient)
      ...
      if (error instanceof Error) {
        if (error.message.includes('Phone number') && error.message.includes('already registered')) {
          ...
          errorMessage = 'You do not have permission to update patients. Please contact your administrator.'
        } else {
          errorMessage = error.message
        }
      }
