...
import { MainExam, UpdateMainExamRequest } from '@/lib/types'
import { LoadingPage } from '@/components/loading-page'

type SlitRow = { id?: number; structure: string; finding: string; eyeSide: 'R'|'L'|'Both' }

export default function EditMainExamPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setExam(data)
        setRows((data.slitLampObservations || []).map(o => ({ id: o.id, structure: o.structure, finding: o.finding, eyeSide: o.eyeSide as SlitRow['eyeSide'] })))
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSaving(true)
      const payload: UpdateMainExamRequest = {
        id: examId,
      ...
      const result = await mainExamApi.update(examId, payload)
      router.push(`/dashboard/main-exams/${result.id}`)
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Failed to update main exam')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit Main Exam</h1>
        <p className="text-gray-600">Exam #{exam.id} • Visit Session #{exam.visitSessionId}</p>
      </div>
        ...
        <div className="bg-white shadow rounded-lg p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.01" value={exam.cdrRight ?? ''} onChange={e => setExam(prev => prev ? { ...prev, cdrRight: e.target.value ? parseFloat(e.target.value) : undefined } : prev)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">CDR Left</label>
              <input type="number" step="0.01" value={exam.cdrLeft ?? ''} onChange={e => setExam(prev => prev ? { ...prev, cdrLeft: e.target.value ? parseFloat(e.target.value) : undefined } : prev)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">IOP Right</label>
              <input type="number" step="0.1" value={exam.iopRight ?? ''} onChange={e => setExam(prev => prev ? { ...prev, iopRight: e.target.value !== '' ? parseFloat(e.target.value) : undefined } : prev)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">IOP Left</label>
              <input type="number" step="0.1" value={exam.iopLeft ?? ''} onChange={e => setExam(prev => prev ? { ...prev, iopLeft: e.target.value !== '' ? parseFloat(e.target.value) : undefined } : prev)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea value={exam.advice || ''} onChange={e => setExam(prev => prev ? { ...prev, advice: e.target.value } : prev)} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">History Comments</label>
              <textarea value={exam.historyComments || ''} onChange={e => setExam(prev => prev ? { ...prev, historyComments: e.target.value } : prev)} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Doctor's Notes</label>
            <textarea value={exam.doctorsNotes || ''} onChange={e => setExam(prev => prev ? { ...prev, doctorsNotes: e.target.value } : prev)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Time Completed</label>
            <input type="datetime-local" value={exam.timeCompleted || ''} onChange={e => setExam(prev => prev ? { ...prev, timeCompleted: e.target.value } : prev)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
          </div>
        </div>

        <div className="flex justify-end">
          <LoadingButton type="submit" loading={isSaving} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700">
